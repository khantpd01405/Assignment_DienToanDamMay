...
if (isset($_POST['kh']))
{
      ...
      echo "<script>$('#a1').attr('class', 'active');</script>";
    }
  }else{
    echo "<script>$('#a1').removeAttr('class');</script>";
  }
  if(isset($_POST['lsp'])){
...
function select($name){
  global $conn, $kh_text, $lsp_text, $sp_text, $hd_text, $cthd_text;
  if($name == "khach_hang"){
    $i=0;
    ...
    while($row = $result->fetch_assoc()){
        ...
        $i++;
         ...
         ."<a href='delete.php?id=".$row['ma_kh']."&name=1' id='kh_bt$i' class='waves-effect waves-light btn'> <i class='material-icons'>delete_forever</i></a></td></tr>";
      ...
      if($name == "san_pham"){
       ...
       if($name == "hoa_don"){
        ...
        if($name == "chitiet_hoadon"){
         ...
         while($row = $result->fetch_assoc()){
           $i++;
           $cthd_text .= "<tr id='cthd$i'><td>".$row["ma_chitiethd"]."</td><td>".$row["Hoa_don_ma_hd"]."</td>"
           ."<td>".$row["San_pham_ma_sp"]."</td><td>".$row["soluongmua"]."</td><td>".$row["gia_ban"]."</td>".
